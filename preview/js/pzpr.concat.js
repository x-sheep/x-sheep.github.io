...
var _doc = "undefined" != typeof document ? document : void 0, _2PI = 2 * Math.PI, _color = [], _hex = function () {
    for (var a = [], b = 256; b < 512; b++)
        a[b - 256] = b.toString(16).substr(1);
    return a
}
(), Candle = {
    version: "0.8.2",
    env: {
        node: "object" == typeof module && "object" == typeof exports && "function" == typeof require,
        browser: "object" == typeof document && "object" == typeof window && "object" == typeof location
    },
    wrapper: {},
    addWrapper: function (a, b) {
        var c,
        d = function () {
            this.initialize && this.initialize.apply(this, arguments)
        };
        for (c in this.wrapperbase)
            d.prototype[c] = this.wrapperbase[c];
        for (c in b)
            d.prototype[c] = b[c];
        this.wrapper[a] = d.prototype.constructor = d,
        this.addType(a)
    },
    _order: [],
    enable: {},
    ...
    TypeList: function (a) {
        for (var b = 0; b < Candle._order.length; b++)
            this[Candle._order[b]] = Candle._order[b] === a
    },
    current: "",
    select: function (a) {
        return !!this.enable[a] && (this.current = a, !0)
    },
    ME: null,
    initME: function () {
        ...
        void 0 !== a.offsetHeight ? this.ME = a : _doc.body.removeChild(a)
    },
    getoffsetHeight: function (a, b) {
        var c;
        if (b.match(/(.+\s)?([0-9]+)px (.+)$/))
            c = +RegExp.$2;
        else if (this.ME) {
    ...
    parse: function (a) {
        if (!_color[a])
            if ("rgb(" === a.substr(0, 4)) {
                var b = a.match(/\d+/g);
                _color[a] = ["#", _hex[b[0]], _hex[b[1]], _hex[b[2]]].join("")
            } else
                _color[a] = a;
        return _color[a]
    },
    getRectSize: function (a) {
        return {
            width: a.offsetWidth || a.clientWidth || 0,
            height: a.offsetHeight || a.clientHeight || 0
        }
    },
    _counter: -1,
    getcanvasid: function () {
        return this._counter++,
        "_candle_" + this._counter
    },
    start: function (a, b, c) {
        this.ME || "undefined" == typeof window || this.initME();
        var d;
        if (a.getContext)
            d = a.getContext("2d");
        else {
            var e = b;
            if (this.enable[e] || (e = this.current), !e || !this.enable[e])
                throw "No canvas environment is installed";
            d = new this.wrapper[e](a)
        }
        c && c(d)
    }
};
"object" == typeof module && "object" == typeof exports ? module.exports = Candle : this.Candle = Candle, function () {
    function a() {
        if (this.parentNode) {
            var a = this.parentNode._children,
            b = a.indexOf(this);
    ...
    function c(a) {
        ...
        return b >= 0 && (this._children.splice(b, 1), a.parentNode = null),
        a
    }
    function d(a, b) {
        a.parentNode && a.parentNode.removeChild(a);
        var c = -1;
        b && (c = this._children.indexOf(b)),
        c === -1 ? this.appendChild(a) : this._children.splice(c, 0, a)
    }
    function e(a, b) {
        a = a.replace(/\*/g, "[^/]+"),
        a = a.replace(/([\w#]+|\*)/g, "[^/]*$1[^/]*"),
        a = a.replace(/\s+/g, "/([^/]+/)?"),
        a = new RegExp(a + "[^/]*$");
        for (var c = this.firstChild ? [this.firstChild] : [], d = []; c.length > 0; ) {
            var e = c.pop();
            if (h(e, this).match(a) && (d.push(e), b))
				break;
			if(!e.childNodes)
				console.log(e);
            e.nextSibling && c.push(e.nextSibling),
            e.childNodes.length > 0 && c.push(e.firstChild)
        }
        return d
    }
    function f(a) {
    ...
    function g(a) {
        return e.call(this, a, !0)[0] || null
    }
    function h(a, b) {
        for (var c = ""; a && a !== b; ) {
            var d = "";
            a.createElement || (d = a.tagName + (a._attr.id ? "#" + a._attr.id : "")),
            c = d + (c ? "/" + c : ""),
            a = a.parentNode
        }
        return c
    ...
    function i() {
        return this._children.reduce(function (a, b, c, d) {
    ...
    function j() {
        ...
        b = "",
        c = "";
        for (var d in this._attr)
            b += " " + d + '="' + this._attr[d] + '"';
        for (var e in this.style)
            c += e + ":" + this.style[e] + ";";
        return c && (b += ' style="' + c + '"'),
        0 === this._children.length ? "<" + a + b + "/>" : "<" + a + b + ">" + this.innerHTML + "</" + a + ">"
    }
    function k(a, b) {
        for (; ; ) {
            var c = b.match(/^\s*([^:]+?):([^;]+?);?/);
    ...
    function l(a, b) {
        for (; ; ) {
            var c = b.match(/^\s*([\w\-\:]+)(\=)?(?:['"](.+?)['"]|([^\s]+))?\s*/);
            if (!c)
                break;
            var d = c[1],
            e = c[3] || c[4] || "";
            b = RegExp["$'"],
            d && ("=" === c[2] ? "style" !== d ? a.setAttribute(d, e) : a.style = k(a, e) : a.setAttribute(d, !0))
        }
    }
    function m(a, b) {
        var c = [a];
        return b.split(/(<.+?>)/g).forEach(function (a) {
            var b = c[c.length - 1],
            d = null;
            if (!a.match(/^<\?xml/))
                if (a.match(/^<\w/)) {
    ...
    n.prototype = {
        ...
        getAttribute: function (a) {
            return a in this._attr ? this._attr[a] : null
        },
        getAttributeNS: function (a, b) {
            return b in this._attr ? this._attr[b] : null
        },
    ...
    var p = Candle.MockDocument = function () {
        this._children = []
    };
    p.prototype = {
        _children: null,
    ...
    var q = Candle.MockXMLSerializer = function () {};
    q.prototype.serializeToString = function (a) {
        return a.outerHTML
    };
    var r = Candle.MockDOMParser = function () {};
    r.prototype.parseFromString = function (a, b) {
        return m(new p, a)
    },
    _doc || (_doc = new p),
    Candle.document = _doc,
    Candle.XMLSerializer = q,
    Candle.DOMParser = r
}
(), Candle.wrapperbase = {
    initialize: function (a) {
        ...
        this.child = null,
        this.enableTextLengthWA = !1,
        this.initElement(),
        this.initFunction(),
        this.initLayer();
        var b = this;
        this.canvas.getContext = function (a) {
            return b
        }
    },
    rectcenter: function (a, b, c, d) {
        this.rect(a - c, b - d, 2 * c, 2 * d)
    },
    fillRectCenter: function (a, b, c, d) {
        this.fillRect(a - c, b - d, 2 * c, 2 * d)
    },
    strokeRectCenter: function (a, b, c, d) {
        this.strokeRect(a - c, b - d, 2 * c, 2 * d)
    },
    shapeRectCenter: function (a, b, c, d) {
        this.shapeRect(a - c, b - d, 2 * c, 2 * d)
    },
    vhide: function () {}
}, function () {
    function a(a) {
        return _doc.createElementNS(d, a)
    }
    function b() {
    ...
    if (void 0 !== _doc && _doc.createElementNS && ("object" != typeof window || !window.opera)) {
        var c,
        d = Candle.SVGNS = "http://www.w3.org/2000/svg",
        e = Candle.XLINKNS = "http://www.w3.org/1999/xlink",
        f = function (a) {
            return a.match(/Trident\//) || a.match(/Safari\//) && a.match(/Edge\//)
        }
        ...
        Candle.addWrapper("svg", {
            initialize: function (a) {
                ...
                this.lastpath = "",
                this.freezepath = !1,
                Candle.wrapperbase.initialize.call(this, a),
                this.enableTextLengthWA = f
            },
            initElement: function () {
                Candle.env.browser && (this.canvas.style.overflow = "hidden");
                var a = Candle.getRectSize(this.canvas),
                b = this.child = _doc.createElementNS(d, "svg");
                b.setAttribute("xmlns", d),
                b.setAttribute("xmlns:xlink", e),
                b.setAttribute("font-size", "10px"),
                b.setAttribute("font-family", "sans-serif"),
                b.setAttribute("width", a.width),
                b.setAttribute("height", a.height),
                b.setAttribute("viewBox", [0, 0, a.width, a.height].join(" ")),
                this.canvas.appendChild && this.canvas.appendChild(b)
            },
            initFunction: function () {
                function a(a) {
                    return Candle.env.browser ? window.btoa(a) : Buffer.isBuffer(a) ? a.toString("base64") : new Buffer(a.toString(), "binary").toString("base64")
                }
                function b(a) {
                    return (a.outerHTML || (new Candle.XMLSerializer).serializeToString(a)).replace(/^<\?xml.+?\?>[\r\n]*/, "")
                }
                var c = '<?xml version="1.0" encoding="UTF-8"?>\n',
                d = this.child;
                this.canvas.toDataURL = function (c, e) {
                    return "data:image/svg+xml;base64," + a(b(d))
                },
                this.canvas.toBlob = function (a, e, f) {
                    a(new Blob([c + b(d)], {
                            type: "image/svg+xml"
                        }))
                },
                this.canvas.toBuffer = function (a, e) {
                    return c + b(d)
                }
            },
            initLayer: function () {
                this.setLayer();
                var a = Candle.getRectSize(this.canvas);
                this.rect(0, 0, a.width, a.height),
                this.addVectorElement(!1, !1)
            },
            clear: function () {
                for (var a = this.child, b = a.firstChild; b; )
            ...
            setLayer: function (b, c) {
                if (c = c || {}, this.vid = "", b) {
                    var d = this.layers[b];
                    d || (d = this.layers[b] = a("g"), this.child.appendChild(d)),
                    this.target = d
                } else
                    this.target = this.child;
                c.rendering && this.setRendering(c.rendering),
                this.freezepath = !!c && c.freeze
            },
            setRendering: function (a) {
                this.target.setAttribute("shape-rendering", a)
            },
            changeSize: function (a, b) {
                Candle.env.browser && (this.canvas.style.width = a + "px", this.canvas.style.height = b + "px");
                var c = this.child;
                c.setAttribute("width", a),
                c.setAttribute("height", b);
                var d = c.getAttribute("viewBox").split(/ /);
                c.setAttribute("viewBox", [d[0], d[1], a, b].join(" "))
            },
            translate: function (a, b) {
                var c = this.child.getAttribute("viewBox").split(/ /);
                c[0] = -a,
                c[1] = -b,
                this.child.setAttribute("viewBox", c.join(" "))
            },
            beginPath: function () {
                this.cpath = [],
                this.lastpath = ""
            ...
            moveTo: function (a, b) {
                this.cpath.push("M", a, b),
                this.lastpath = "M"
            },
            lineTo: function (a, b) {
                "L" !== this.lastpath && this.cpath.push("L"),
                this.cpath.push(a, b),
                this.lastpath = "L"
            },
            rect: function (a, b, c, d) {
                this.cpath.push("M", a, b, "L", a + c, b, a + c, b + d, a, b + d, "z"),
                this.lastpath = "z"
            },
            arc: function (a, b, c, d, e, f) {
                ...
                i,
                j;
                e - d >= _2PI ? (g = a + c, h = b, i = a + c, j = b) : (g = a + c * Math.cos(d), h = b + c * Math.sin(d), i = a + c * Math.cos(e), j = b + c * Math.sin(e)),
                e - d >= _2PI && (h += .125);
                var k = d > e != Math.abs(e - d) > Math.PI,
                l = f ^ k ? 1 : 0,
                m = 0 === l ^ k ? 1 : 0;
                this.cpath.push("M", g, h, "A", c, c, 0, l, m, i, j),
                this.lastpath = "A"
            },
            fill: function () {
                this.addVectorElement(!0, !1)
            },
            stroke: function () {
                this.addVectorElement(!1, !0)
            },
            shape: function () {
                this.addVectorElement(!0, !0)
            },
            ...
            setLinePath: function () {
                for (var a = arguments, b = a.length, c = b - (1 | b ? 1 : 2), d = [], e = 0; e < c; e += 2)
                    d[e >> 1] = [a[e], a[e + 1]];
                this.beginPath(),
                this.setLinePath_com.call(this, d),
                a[b - 1] && this.cpath.push("z")
            },
            setOffsetLinePath: function () {
                for (var a = arguments, b = a.length, c = b - (1 | b ? 1 : 2), d = [], e = 0; e < c - 2; e += 2)
                    d[e >> 1] = [a[e + 2] + a[0], a[e + 3] + a[1]];
                this.beginPath(),
                this.setLinePath_com.call(this, d),
                a[b - 1] && this.cpath.push("z")
            },
            setLinePath_com: function (a) {
                for (var b = 0, c = a.length; b < c; b++)
                    this.cpath.push(0 === b ? "M" : "L"), this.cpath.push(a[b][0], a[b][1])
            },
            strokeLine: function (a, b, c, d) {
                var e = this.cpath;
                this.cpath = ["M", a, b, "L", c, d],
                this.addVectorElement(!1, !0),
                this.cpath = e
            },
            strokeDashedLine: function (a, b, c, d, e) {
                var f = this.cpath;
                this.cpath = ["M", a, b, "L", c, d],
                this.addVectorElement(!1, !0).setAttribute("stroke-dasharray", e.join(" ")),
                this.cpath = f
            },
            strokeCross: function (a, b, c) {
                var d = this.cpath;
                this.cpath = ["M", a - c, b - c, "L", a + c, b + c, "M", a - c, b + c, "L", a + c, b - c],
                this.addVectorElement(!1, !0),
                this.cpath = d
            },
            ...
            getDefsElement: function () {
                var a = this.child.querySelector("defs");
                return a || (a = _doc.createElementNS(d, "defs"), this.child.insertBefore(a, this.child.firstChild || null)),
                a
            },
            getImageElement: function (b) {
                for (var c = this.getDefsElement(), d = null, f = c.querySelectorAll("image"), g = 0; g < f.length; g++)
                    if (f[g].getAttributeNS(e, "href") === b.src) {
                        d = f[g];
                        break
                    }
                return d || (d = a("image"), d.setAttribute("id", (d.ownerDocument ? this.canvasid + "_" : "") + "img" + f.length), d.setAttribute("width", b.width), d.setAttribute("height", b.height), d.setAttributeNS(e, "xlink:href", b.src), c.appendChild(d)),
                d
            },
            getImageSymbol: function (a, b, c, f, g) {
                for (var h = this.getDefsElement(), i = [b, c, f, g].join(" "), j = null, k = h.querySelectorAll("symbol"), l = 0; l < k.length; l++)
                    if (k[l].getAttribute("viewBox") === i) {
                        j = k[l];
                        break
                    }
                if (!j) {
                    j = _doc.createElementNS(d, "symbol"),
                    j.setAttribute("id", (j.ownerDocument ? this.canvasid + "_" : "") + "symimg" + k.length),
                    j.setAttribute("viewBox", i);
                    var m = _doc.createElementNS(d, "use");
                    m.setAttributeNS(e, "xlink:href", "#" + this.getImageElement(a).getAttribute("id")),
                    j.appendChild(m),
                    h.appendChild(j)
                }
                return j
            ...
            fillText: function (a, b, c, d) {
                var e = this.vid ? this.elements[this.vid] : null;
                if (a && this.fillStyle && "none" !== this.fillStyle) {
                    var f = this.fillText_main(e, a, b, c, d || "");
                    !e && this.vid && (this.elements[this.vid] = f)
                } else
                    e && this.hide(e);
                this.vid = ""
            },
            fillText_main: function (b, d, e, h, i) {
                var j = !b,
                k = this.vid ? this._textcache[this.vid] || {}
                 : {};
                if (j ? b = a("text") : this.show(b), b.getAttribute("fill") !== this.fillStyle && b.setAttribute("fill", this.fillStyle), k.x !== e || k.y !== h || k.ml !== i || k.ta !== this.textAlign || k.tb !== this.textBaseline || k.font !== this.font) {
                    var l = h - Candle.getoffsetHeight(d, this.font) * c[this.textBaseline.toLowerCase()],
                    m = g[this.textAlign.toLowerCase()];
                    b.getAttribute("x") !== e && b.setAttribute("x", e),
                    b.getAttribute("y") !== l && b.setAttribute("y", l),
                    b.getAttribute("text-anchor") !== m && b.setAttribute("text-anchor", m),
                    (b.getAttribute("textLength") || "") !== i && (i ? (b.setAttribute("textLength", i), b.setAttribute("lengthAdjust", "spacingAndGlyphs")) : (b.removeAttribute("textLength"), b.removeAttribute("lengthAdjust"))),
                    k.x = e,
                    k.y = h,
                    k.ml = i,
                    k.ta = this.textAlign,
                    k.tb = this.textBaseline
                }
                if (k.font !== this.font) {
                    if (this.font.match(/(.+\s)?([0-9]+)px (.+)$/)) {
                        ...
                        o = RegExp.$2,
                        p = RegExp.$3;
                        b.setAttribute("font-size", o),
                        p.match(/^sans\-serif$/i) ? b.removeAttribute("font-family") : b.setAttribute("font-family", p),
                        n.match(/(italic|oblique)/) ? b.setAttribute("font-style", RegExp.$1) : b.removeAttribute("font-style"),
                        n.match(/(bold|bolder|lighter|[1-9]00)/) ? b.setAttribute("font-weight", RegExp.$1) : b.removeAttribute("font-weight")
                    } else
                        b.setAttribute("font", this.font);
                    k.font = this.font
                }
                if (b.textContent !== d && (b.textContent = d), this.vid && (this._textcache[this.vid] = k), j && this.target.appendChild(b), f && this.enableTextLengthWA && ("center" === k.ta || "right" === k.ta) && i) {
                    b.removeAttribute("textLength");
                    var q = b.getBoundingClientRect();
                    b.setAttribute("x", k.x + (q.width - i) / ("center" === k.ta ? 2 : 1)),
                    b.setAttribute("textLength", i)
                }
                return b
            },
            drawImage: function (a, b, c, d, e, f, g, h, i) {
                var j = this.vid ? this.elements[this.vid] : null;
                if (a) {
                    void 0 === d && (d = a.width, e = a.height),
                    void 0 === f && (f = b, b = 0, g = c, c = 0, h = d, i = e);
                    var k = this.drawImage_main(j, a, b, c, d, e, f, g, h, i);
                    !j && this.vid && (this.elements[this.vid] = k)
                } else
                    j && this.hide(j);
                this.vid = ""
            },
            drawImage_main: function (b, c, d, f, g, h, i, j, k, l) {
                var m = !b;
                m ? b = a("use") : this.show(b);
                var n = this.getImageSymbol(c, d, f, g, h).getAttribute("id");
                return b.getAttribute("x") !== i && b.setAttribute("x", i),
                b.getAttribute("y") !== j && b.setAttribute("y", j),
                b.getAttribute("width") !== k && b.setAttribute("width", k),
                b.getAttribute("height") !== l && b.setAttribute("height", l),
                b.getAttributeNS(e, "xlink:href") !== "#" + n && b.setAttributeNS(e, "xlink:href", "#" + n),
                m && this.target.appendChild(b),
                b
            },
            addVectorElement: function (a, b) {
                a = a && !!this.fillStyle && "none" !== this.fillStyle,
                b = b && !!this.strokeStyle && "none" !== this.strokeStyle;
                var c = this.vid ? this.elements[this.vid] : null,
                d = null;
                return a || b ? (d = this.addVectorElement_main(c, a, b), !c && this.vid && (this.elements[this.vid] = d)) : c && this.hide(c),
                this.vid = "",
                d
            },
            addVectorElement_main: function (b, c, d) {
                var e = !b;
                if (e ? (b = a("path"), b.setAttribute("fill", "none"), b.setAttribute("stroke", "none")) : this.show(b), !this.freezepath || e) {
                    var f = this.cpath.join(" "),
                    g = d ? this.lineWidth : null;
                    b.getAttribute("d") !== f && b.setAttribute("d", f),
                    b.getAttribute("stroke-width") !== g && b.setAttribute("stroke-width", g)
                }
                var h = c ? this.fillStyle : "none",
                i = d ? this.strokeStyle : "none";
                return b.getAttribute("fill") !== h && b.setAttribute("fill", h),
                b.getAttribute("stroke") !== i && b.setAttribute("stroke", i),
                e && this.target.appendChild(b),
                b
            },
            vhide: function (a) {
                var b = this.elements[this.vid];
                b && this.hide(b)
            },
            show: function (a) {
...
(), function () {
    function a() {
        var a = "undefined" != typeof navigator ? navigator.userAgent || "" : "";
        b = 0,
        b = a.match(/Chrome/) ?  - .72 : a.match(/AppleWebKit/) ?  - .7 : a.match(/Trident/) ?  - .74 : a.match(/Win/) ?  - .7 :  - .76
    }
    if (Candle.env.browser) {
        if (!function () {
            var a = _doc.createElement("canvas");
            return !!a.getContext && (!a.probablySupportsContext || a.probablySupportsContext("2d"))
        }
            ())
            return
    } else
        try {
            Candle.Canvas = require("canvas")
        } catch (a) {
            return
        }
    var b;
    Candle.addWrapper("canvas", {
        initialize: function (c) {
            ...
            this.x0 = 0,
            this.y0 = 0,
            Candle.wrapperbase.initialize.call(this, c)
        },
        setkey: function (a) {
            return this
        },
        ...
        initElement: function () {
            var a = this.child = Candle.env.browser ? _doc.createElement("canvas") : new Candle.Canvas;
            Candle.env.browser && (this.canvas.style.overflow = "hidden");
            var b = Candle.getRectSize(this.canvas);
            a.width = b.width,
            a.height = b.height,
            Candle.env.browser && (a.style.width = b.width + "px", a.style.height = b.height + "px", this.canvas.appendChild(a)),
            this.context = a.getContext("2d")
        },
        initFunction: function () {
            function a(a) {
                return Candle.env.browser ? window.atob(a) : new Buffer(RegExp.$2, "base64").toString("binary")
            }
            var b = this.child;
            this.canvas.toDataURL = function (a, c) {
                return b.toDataURL(a || void 0, c)
            },
            this.canvas.toBlob = function (c, d, e) {
                if ("function" == typeof b.toBlob)
                    b.toBlob(c, d, e);
                else {
                    b.toDataURL(d || void 0, e).match(/data:(.*);base64,(.*)/);
                    for (var f = a(RegExp.$2), g = f.length, h = new Uint8Array(g), i = 0; i < g; i++)
                        h[i] = f.charCodeAt(i);
                    c(new Blob([h.buffer], {
                            type: RegExp.$1
                        }))
            ...
            this.canvas.toBuffer = function (c, d) {
                var e = b.toDataURL(c || void 0, d).replace(/^data:image\/\w+?;base64,/, "");
                if (Candle.env.node)
        ...
        clear: function () {
            if (this.setProperties(!0, !0), this.context.setTransform(1, 0, 0, 1, 0, 0), this.context.translate(this.x0, this.y0), Candle.env.browser) {
                var a = Candle.getRectSize(this.canvas);
                this.context.clearRect(0, 0, a.width, a.height)
            }
        },
        setLayer: function (a, b) {
            var c = this.currentLayerId = a ? a : "_empty";
            this.isedge = this.isedgearray[void 0 !== this.isedgearray[c] ? c : "_empty"],
            this.setEdgeStyle(),
            b = b || {},
            b.rendering && this.setRendering(b.rendering)
        },
        setEdgeStyle: function () {
            if (Candle.env.browser) {
                var a = this.canvas.style;
                "imageRendering" in a && (a.imageRendering = "", this.isedge && (a.imageRendering = "pixelated", a.imageRendering || (a.imageRendering = "-webkit-optimize-contrast"), a.imageRendering || (a.imageRendering = "-moz-crisp-edges"), a.imageRendering || (a.imageRendering = "-o-crisp-edges")))
            }
        },
        setRendering: function (a) {
            this.isedge = this.isedgearray[this.currentLayerId] = "crispEdges" === a,
            this.setEdgeStyle()
        },
        changeSize: function (a, b) {
            if (Candle.env.browser) {
        ...
        setProperties: function (a, b) {
            ...
            b = b && !!this.strokeStyle && "none" !== this.strokeStyle;
            var c = this.context;
            return a && (c.fillStyle = this.fillStyle),
            b && (c.strokeStyle = this.strokeStyle),
        ...
        fill: function () {
            this.setProperties(!0, !1) && this.context.fill()
        },
        stroke: function () {
            this.setProperties(!1, !0) && this.context.stroke()
        },
        shape: function () {
            if (this.setProperties(!0, !0)) {
                var a = this.context;
                this.fillStyle && "none" !== this.fillStyle && a.fill(),
                this.strokeStyle && "none" !== this.strokeStyle && a.stroke()
            }
        },
        fillRect: function (a, b, c, d) {
        ...
        shapeRect: function (a, b, c, d) {
            if (this.setProperties(!0, !0)) {
                var e = this.context;
                this.fillStyle && "none" !== this.fillStyle && e.fillRect(a, b, c, d),
        ...
        setLinePath: function () {
            ...
            this.context.beginPath(),
            this.setLinePath_com.call(this, d),
            a[b - 1] && this.context.closePath()
        },
        setOffsetLinePath: function () {
            for (var a = arguments, b = a.length, c = b - (1 | b ? 1 : 2) - 2, d = [], e = 0; e < c; e += 2)
                d[e >> 1] = [a[e + 2] + a[0], a[e + 3] + a[1]];
            this.context.beginPath(),
            this.setLinePath_com.call(this, d),
            a[b - 1] && this.context.closePath()
        ...
        setLinePath_com: function (a) {
            for (var b = 0, c = a.length; b < c; b++) {
                var d = a[b];
                0 === b ? this.context.moveTo(d[0], d[1]) : this.context.lineTo(d[0], d[1])
            }
        },
        strokeLine: function (a, b, c, d) {
            if (this.setProperties(!1, !0)) {
                var e = this.context;
                e.beginPath(),
                e.moveTo(a, b),
                e.lineTo(c, d),
        ...
        strokeDashedLine: function (a, b, c, d, e) {
            this.strokeDashedLine = this.context.setLineDash ? function (a, b, c, d, e) {
                var f = this.context;
                this.setProperties(!1, !0) && (f.beginPath(), f.moveTo(a, b), f.lineTo(c, d), f.setLineDash(e), f.stroke(), f.setLineDash([]))
            }
             : function (a, b, c, d, e) {
                e.length % 2 === 1 && (e = e.concat(e));
                var f = Math.sqrt((c - a) * (c - a) + (d - b) * (d - b)),
                g = 0,
                h = 0,
                i = null,
                j = null;
                if (a !== c && (i = (d - b) / (c - a), j = i * i + 1), this.setProperties(!1, !0)) {
                    var k = this.context;
                    for (k.beginPath(), k.moveTo(a, b); g < f; ) {
                        var l,
                        m;
                        if (null !== i) {
                            var n = Math.sqrt(g * g / j);
                            l = a + n,
                            m = b + i * n
                        } else
                            l = a, m = b + g;
                        0 === (1 & h) ? k.moveTo(l, m) : k.lineTo(l, m),
                        g += e[h],
                        h++,
                        h >= e.length && (h = 0)
                    }
            ...
            this.strokeDashedLine(a, b, c, d, e)
        },
        strokeCross: function (a, b, c) {
            if (this.setProperties(!1, !0)) {
                var d = this.context;
        ...
        fillCircle: function (a, b, c) {
            if (this.setProperties(!0, !1)) {
                var d = this.context;
                d.beginPath(),
                d.arc(a, b, c, 0, _2PI, !1),
                d.fill()
            }
        },
        strokeCircle: function (a, b, c) {
        ...
        shapeCircle: function (a, b, c) {
            if (this.setProperties(!0, !0)) {
                ...
                d.beginPath(),
                d.arc(a, b, c, 0, _2PI, !1),
                this.fillStyle && "none" !== this.fillStyle && d.fill(),
                this.strokeStyle && "none" !== this.strokeStyle && d.stroke()
            }
        },
        fillText: function (a, c, d, e) {
            a && this.setProperties(!0, !1) && ("candle-top" === this.textBaseline && (d -= Candle.getoffsetHeight(a, this.font) * b, this.context.textBaseline = "alphabetic"), e ? this.context.fillText(a, c, d, e) : this.context.fillText(a, c, d))
        },
        drawImage: function () {
